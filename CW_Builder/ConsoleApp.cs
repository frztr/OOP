...
public class ConsoleApp
{
      ...
      )
    {
        ...
        if (fileRepoNeeded)
            arguments.Add("-file");
        if (extSearch != "")
            arguments.Add("-extendedsearch");
        arguments.AddRange(extSearch.Split(" "));
        ...
        var appDbContextCs = AppDbContextCreator.Create(entities.ToArray(), host, port, dbName, hostUser, password);
        ...
        if (System.OperatingSystem.IsLinux())
        {
            commands = new List<List<string>>(){
                new List<string>(){"dotnet",@"new tool-manifest","/Repositories"},
                new List<string>(){"dotnet",@"tool install --local dotnet-ef","/Repositories"},
                new List<string>(){"dotnet","dotnet-ef migrations add Init","/Repositories"},
                new List<string>(){"dotnet","dotnet-ef database update","/Repositories"}
            };
        }
        else
        {
            commands = new List<List<string>>(){
                new List<string>(){"dotnet",@"tool install --global dotnet-ef","/"},
                new List<string>(){"dotnet-ef","migrations add Init","/Repositories"},
                new List<string>(){"dotnet-ef","database update","/Repositories"}
            };
        }
